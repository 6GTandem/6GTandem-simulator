...
- This can be used to debug the ideal scenario without any hardware impairments or effects from the wireless channel.
...
if __name__ == "__main__":
    ...
    # sanity check
    tx_freq_oversampled = wf.ofdm_time_to_freq(ofdm_time)
    ...
    logger.debug("pilot indices:", wf.pilot_indices)
    logger.debug("pilot values (TX) for first symbol:", tx_subc[0, wf.pilot_indices])
    logger.debug("pilot magnitudes mean:", np.mean(np.abs(tx_subc[:, wf.pilot_indices])))
    ...
    # Setting debug=True enables a dummy channel of all ones.
